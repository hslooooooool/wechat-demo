...
abstract class BaseAdapter<T>(
...
) : RecyclerView.Adapter<BaseHolder<T>>(), OnRecyclerViewItemClickListener {

    var mHolder: BaseHolder<T>? = null
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): BaseHolder<T> {
        ...
        view = if (viewId == null) {
            ...
            try {
                LayoutInflater.from(mContext).inflate(viewId, parent, false)
            } catch (e: Exception) {
                View(mContext)
            }
        }
        mHolder = getHolder(view, viewType)
        if (mHolder == null) return BaseNullHolder(view)
